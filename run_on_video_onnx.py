...
from general import postprocess  # Assuming this function is defined elsewhere
...
def infer_model(frames, onnx_session):
    """ Run ONNX model on a consecutive list of frames
    :params
    ...
    for num in tqdm(range(2, len(frames))):
        ...
        output = np.argmax(ort_outs[0], axis=1)
...
if __name__ == '__main__':
    ...
    # Load ONNX model
    onnx_session = onnxruntime.InferenceSession(args.model_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])

    frames, fps = read_video(args.video_path)
    ball_track, dists = infer_model(frames, onnx_session)
